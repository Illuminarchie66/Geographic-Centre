...
class Centre_Calculator:
    ...
    def centre(self, V, experimental=False, verbose=True):
        if len(V) < 1:
            return None

        if experimental:
            print("Centre N")
            return self.centreN(V)
    ...
    def centreN(self, V):
        ...
        while not np.allclose(C, C_next, atol=precision):
            ...
            delta = 2*((1/n)*s1 - (1/np.square(n))*s2*s3)

            C_next = C - alpha*delta*C
...
# P1 = Point.fromPolar([0,0])
# P2 = Point.fromPolar([0,15])
# P3 = Point.fromPolar([15,15])
# P4 = Point.fromPolar([15,0])

# C2 = calc.centreN([P1,P2,P3,P4])
# print(C2)
# print(calc.format(C2, [P1, P2, P3, P4]))